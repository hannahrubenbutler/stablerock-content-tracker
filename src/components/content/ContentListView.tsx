...
export default function ContentListView({ onRequestClick, serviceFilter, contentFilter }: ContentListViewProps) {
  ...
  const filtered = useMemo(() => {
    return tabRequests.filter((r) => {
      ...
      if (search) {
        ...
        const matchPerson = (r as any).contact_person?.toLowerCase().includes(q);
        const matchSubmitter = r.submitter_name?.toLowerCase().includes(q);
        if (!matchTitle && !matchDesc && !matchPerson && !matchSubmitter) return false;
      }
      return true;
    }).sort((a, b) => {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <input
          ...
          className="text-xs font-body bg-card border border-border rounded px-3 py-1.5 text-foreground w-48 placeholder:text-muted-foreground"
        />
